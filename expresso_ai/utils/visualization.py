...
def _normalize_video_attr(
    attr: ndarray, sign: str, outlier_perc: Union[int, float] = 2
):
    attr_combined = np.sum(attr, axis=0)
    # Choose appropriate signed values and rescale, removing given outlier percentage.
    if VisualizeSign[sign] == VisualizeSign.all:
        threshold = _cumulative_sum_threshold(np.abs(attr_combined), 100 - outlier_perc)
    elif VisualizeSign[sign] == VisualizeSign.positive:
...
def visualize_video_attribution(
    attr: ndarray, 
    fps:int,
    video_array: Union[None, ndarray] = None,
    method: str = "heat_map",
    ...
    norm_attr = _normalize_video_attr(attr, sign, outlier_perc)
    ...
    ax = fig.add_subplot()
    #import pdb; pdb.set_trace()
    for frame, frame_attr in zip(np.moveaxis(video_array, 1, 0), norm_attr):
        #generate attributions for each frame
        frame = np.transpose((frame / 2) + 0.5, (1,2,0))
        #frame_attr = np.transpose(frame_attr, (1,2,0))
        fig, ax, frame_visualization = visualize_video_frame_attr(frame_attr, frame, method=method, sign=sign, outlier_perc=outlier_perc, cmap=cmap, show_colorbar=show_colorbar, use_pyplot=False, plt_fig_axis=(fig, ax))
        output_video.append(frame_visualization)
    #save visualization frames into an animated video object
    output_video_visualization = animation.ArtistAnimation(fig, output_video, interval=fps)
    return output_video_visualization

def visualize_video_frame_attr(
    attr: ndarray,
    ...
    r"""
        Visualizes attribution for a given image by normalizing attribution values across 
    ...
    if ImageVisualizationMethod[method] == ImageVisualizationMethod.original_image:
        original_image_axes = plt_axis.imshow(original_image)
        axes_images = [original_image_axes]
    else:
        #NOTE: Normalization is a pre-processing step done across the video and not per frame
        norm_attr = attr
    ...
    return plt_fig, plt_axis, axes_images
...
def visualize_multiple_image_attr(
    ...
    **kwargs: Any
    ...
    r"""
        ...
        Args:
            ...
            attributions (numpy.array): Numpy array corresponding to attributions to be
                        visualized. Shape must be in the form (k, H, W, C), with
                        channels as last dimension. Shape must also match that of
                        the original image if provided.
            original_image (numpy.array, optional):  Numpy array corresponding to
                        original image. Shape must be in the form (k, H, W, C), with
                        channels as the last dimension. Image can be provided either
                        with values in range 0-1 or 0-255. This is a necessary
                        argument for any visualization method which utilizes
                        the original image.
        ...
        Examples::
            ...
            >>>                     ["all", "positive"], attribution, orig_image)
    """
    assert len(attributions) == len(methods) == len(signs), "Number of attributions, methods and signs array lengths must match."
    if titles is not None:
        assert len(methods) == len(titles), (
            "If titles list is given, length must " "match that of methods list."
        )
    ...
    for i in range(len(attributions)):
        visualize_image_attr(
            attributions[i],
            original_image=original_image[i],
            method=methods[i],
            sign=signs[i],
            plt_fig_axis=(plt_fig, plt_axis[i]),
            use_pyplot=False,
