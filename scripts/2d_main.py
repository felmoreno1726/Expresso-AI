...
def main():
    ...
    model, img_size, img_channels = initialize_model(**vars(args))
    ...
    #layers_to_defreeze = 40#TODO: REMOVE
    #freezer = Freezer(model, defreeze_last_n = layers_to_defreeze)
    freezer = TwoStepFreezer(model, initial_step=4, step_size=1, defreeze_last_n=layers_to_defreeze)
    ...
    if args.train:
        #Train routine
        #for epoch in tqdm(range(epoch, epochs), position=0, total=epochs-epoch):
...
def feed_data(data_loader, phase, model, criterion, epoch, metrics, optimizer=None, scheduler=None):
    ...
    if phase == 'train':
        ...
        if phase == 'interpret':
            ...
            interpreter = ModelInterpreter(model, compute_attributions=args.compute_attributions, algorithms=args.interpretation_algorithms, visualization_methods=args.visualization_methods, signs=args.attribution_signs)
    ...
    for i, (sample_ids, participant_ids, window_index, imFace, labels) in tqdm(enumerate(data_loader), position=1, total=(len(data_loader))):
        ...
        if autograd_flag:
            if phase == 'train':
                ...
                interpreter.interpretation_routine(inputs, input_names, labels.item(), 
                        ...
                        attributions_path=args.attributions_path, interpretation_path=INTERPRETATION_PATH, aus_data_path=AUS_DATA_PATH,#path parameters
                        generate_video_visualizations=args.generate_video_visualizations, generate_outstanding_frames_grid=args.generate_outstanding_frames_grid, generate_interpretation_plots=args.generate_interpretation_plots, generate_action_unit_cross_correlation=args.generate_action_unit_cross_correlation #parameters for routines to execute
                )
                output = model(inputs.squeeze(2))
        else:
            with torch.no_grad():
        ...
        iteration = epoch * len(data_loader) + i if phase != 'test' else i
    ...
    if phase == 'interpret':
        ...
        if args.generate_interpretation_plots:
            #plot accumulated attributions
            interpreter.plot_accumulated_attributions(INTERPRETATION_PATH)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--window_size', type=int, default=10, help="Default=300. The number of frames in each data sample.")
    ...
    parser.add_argument('--input_dilation', type=int, default=10, help="Default=1. The dilation used on the input frames to the model.")
    ...
    parser.add_argument('--interpretation_algorithms', nargs='+', default=['Saliency'], help='Default [\"Saliency\"]. Uses passed in list of interpretations algorithms to run the interpretation routine (Saliency, IntegratedGradients, NoiseTunnel, DeepLift, GuidedGradCam.')
    parser.add_argument('--generate_video_visualizations', type=str2bool, nargs='?', const=True, default=True, help="Default=True. Generates interpretation videos for the generated attributions.")
    parser.add_argument('--generate_outstanding_frames_grid', type=str2bool, nargs='?', const=True, default=True, help="Default=True. Generates interpretation grid of the frames with highest attributions.")
    parser.add_argument('--generate_interpretation_plots', type=str2bool, nargs='?', const=True, default=True, help="Default=True. Generates interpretation plots of the generated attributions.")
    parser.add_argument('--generate_action_unit_cross_correlation', type=str2bool, nargs='?', const=True, default=True, help="Default=True. Action Unit (AU) cross-correlation score with attributions.")
    ...
    parser.add_argument('--model_name', type=str, nargs='?', default="facenet", help="Default=r3d_18. The model name to use (r3d_18, mc3_18, r2plus1d_18, CNN, RNN, CNNAttention).")
    parser.add_argument('--pretrained', type=str, nargs='?', const=True, default='casia-webface', help="Default=casia-webface. If true, loads pretrained weights to the model")
    ...
    comment = f'_model_name={args.model_name}_window_size={args.window_size}_input_dilation={args.input_dilation}_momentum={args.momentum}_weight_decay={args.weight_decay}_batch_size={batch_size}_epochs={args.epochs}_noise_mean={args.noise_mean}_noise_std={args.noise_std}_lr={args.lr}'
    ...
    #max_lr = 1e-4
    min_lr = 0#1e-6
    #NOTE: lr depends on model and architecture. 9e-4 works good on facenet_rcm
